...
const AppContent: React.FC = () => {
  ...
  const handleJoin = () => {
    // This function can be used for a generic join flow, but specific plan selection
    // now routes to the success pages directly from ResultsPage.
    window.location.hash = '#/';
  }

  const renderContent = () => {
    if (location.startsWith('#/success/')) {
        const plan = location.split('/')[2] as 'bronze' | 'silver' | 'gold' | 'founding-member';
        return <SuccessPage plan={plan} />;
    }
